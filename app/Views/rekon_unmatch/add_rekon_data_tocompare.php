...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800"><?= $title; ?> #<?= $_SESSION['tipe']; ?></h1>
    </div>
                  ...
                  <div class="card mb-4">
                      ...
                      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                        <?php if ($_SESSION['tipe'] == "2") {
                           ?>
                          <a href="<?php echo base_url('rekon_unmatch_bulanan/cleansing_data_dua');?>" class="btn btn-danger">Kembali</a>  
                          <a href="<?php echo base_url('rekon_unmatch_bulanan/rekon_preview');?>" class="btn btn-secondary">Next (Preview Data)</a>  
                          <!-- <button type="button" class="btn btn-secondary btnNext" data-bs-toggle="modal" data-bs-target="#staticBackdrop">
                              Next (Preview Data)
                          </button> -->
                        <?php } else {  ?>

                          <a href="<?php echo base_url('rekon_unmatch_bulanan/cleansing_data');?>" class="btn btn-danger">Kembali</a>
                          <a href="<?php echo base_url('rekon_unmatch_bulanan/cleansing_data_dua');?>" class="btn btn-secondary">Next (Data #2)</a>  
                          <!-- <button type="button" class="btn btn-secondary btnNext" data-bs-toggle="modal" data-bs-target="#staticBackdrop" >
                              Next (Data #2)
                          </button> -->
...
<script>
  ...
  $("#addCompare").click(function() {
        ...
        console.log(idt);
        $.ajax({
            url : "<?= base_url('rekon_unmatch_bulanan/add_kolom_compare') ?>",
            method : "POST",
  ...
  $(".rmCompare").click(function() {
      ...
      console.log(idt);
      $.ajax({
          url : "<?= base_url('rekon_unmatch_bulanan/rm_kolom_compare') ?>",
          method : "POST",
  ...
  $(".rmSum").click(function() {
      ...
      $.ajax({
          url : "<?= base_url('rekon_unmatch_bulanan/rm_kolom_sum') ?>",
          method : "POST",
          data : {"rekon_index": indx, "rekon_name": nme},
          async : true,
          success: function($result){
  ...
  $("#addSum").click(function() {      
      ...
      console.log(idt);
      $.ajax({
          url : "<?= base_url('rekon_unmatch_bulanan/add_kolom_sum') ?>",
          method : "POST",
          data : {"rekon_index": id, "rekon_name": idt},
          async : true,
          success: function($result){

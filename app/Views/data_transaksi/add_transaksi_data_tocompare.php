                ...
                <div class="col-12">
                  <div class="card mb-4">
                      <div class="card-header d-flex flex-row align-items-center justify-content-between bg-danger">
                            <h6 class="m-0 font-weight-bold text-light">Manage Data Transaksi</h6>
                      </div>
                      <div class="card-body">
                      ...
                      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                          ...
                          <a href="<?php echo base_url('data_transaksi/cleansing_data');?>" class="btn btn-danger">Kembali</a>  
                          <button type="button" class="btn btn-primary btnNext" data-bs-toggle="modal" data-bs-target="#staticBackdrop" >
                              Finish
                          </button>
    ...
    <!-- <form method="post" enctype="multipart/form-data" action="<?php echo base_url('data_transaksi/upload_with_setting'); ?>"> -->
      ...
      <div class="modal-body">
        ...
        <div class="input-group mb-3">
          <input type="text" class="form-control" placeholder="Setting Name" aria-label="Recipient's username" id="namaSetting_" value="setting rekon <?= $nama_transaksi ?>">
        </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" id="btnSimpan">Finish & Simpan Setting</button>
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" id="btnNextOk">Finish</button>
      </div>
...
<script>
  ...
  $("#btnSimpan").click(function() {
        ...
        $.ajax({
            url : "<?= base_url('save_setting_transaksi') ?>",
            method : "POST",
  ...
  $("#addCompare").click(function() {
        ...
        console.log(idt);
        $.ajax({
            url : "<?= base_url('data_transaksi/add_kolom_compare') ?>",
            method : "POST",
  ...
  $(".rmCompare").click(function() {
      ...
      console.log(idt);
      $.ajax({
          url : "<?= base_url('data_transaksi/rm_kolom_compare') ?>",
          method : "POST",
  ...
  $(".rmSum").click(function() {
      ...
      $.ajax({
          url : "<?= base_url('data_transaksi/rm_kolom_sum') ?>",
          method : "POST",
          data : {"rekon_index": indx, "rekon_name": nme},
          async : true,
          success: function($result){
  ...
  $("#addSum").click(function() {      
      ...
      console.log(idt);
      $.ajax({
          url : "<?= base_url('data_transaksi/add_kolom_sum') ?>",
          method : "POST",
          data : {"rekon_index": id, "rekon_name": idt},
          async : true,
          success: function($result){

                  ...
                  <table class="table align-items-center table-flush" id="dataTableMaster" style="font-size:70% important!;">
                    ...
                    <tbody>
                      ...
                      foreach ($data_rekon as $row) {
                        ...
                        $linkRekonSatu = base_url("rekon_transaksi/export_all/" . $encryptedData1);
                        $linkRekonDua = base_url("rekon_transaksi/export_all/" . $encryptedData2);
                        $linkRekonTiga = base_url("rekon_transaksi/export_all/" . $encryptedData3);
                        $linkRekonEmpat = base_url("rekon_transaksi/export_all/" . $encryptedData4);

                        $idRekonResult = $Encryption->encrypt($row['id_rekon_result'], $key);
                        $linkRetry = base_url("rekon_transaksi/retry_process/" . $idRekonResult);                        
...
<script>
    ...
    $('.btnShowResult').on('click', function(event) {
        ...
        $.ajax({
            url : "<?= base_url('rekon_transaksi/rekon_result_post') ?>",
            method : "POST",
            data : {id_rekon: id_rekon, id_rekon_result: id_rekon_result},
            async : true,
            success: function($hasil){
                window.location.replace("<?= base_url('rekon_transaksi/rekon_result') ?>");
            }
        });
    });
    ...
    $('.btnShowResultAmount').on('click', function(event) {
        ...
        $.ajax({
            ...
            success: function($hasil){
              console.log($hasil)
                window.location.replace("<?= base_url('rekon_transaksi/rekon_result_amount') ?>");
            }
        });
    });
    ...
    //     $.ajax({
    //         url : "<?= base_url('rekon_transaksi/retry_process') ?>",
    //         method : "POST",

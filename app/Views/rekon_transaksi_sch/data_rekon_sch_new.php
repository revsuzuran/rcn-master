...
<script>
  ...
  $("#waktu_rekon").val(waktuSch);   
  $("#nama_rekon").val(namaRekon);  


  const settingSatu = "<?= $data_rekon->detail_schedule->data_satu->setting ?>";
  $("#opt_setting").val(settingSatu).change();
  $("#id_setting").val(settingSatu);

  const settingDua = "<?= $data_rekon->detail_schedule->data_dua->setting ?>";
  $("#opt_setting2").val(settingSatu).change();
  $("#id_setting2").val(settingDua);
  ...
  $('#btnProses').on('click', function(event) {
      ...
      $.ajax({
          url : "<?= base_url('rekon_transaksi_sch/update_rekon') ?>",
          method : "POST",
